    ...
    namespace System {
        ...
        void SysSerialisation::serialize(WorldData &world, Entity::Entity *entity) {
            ...
            for (const auto &componentEntry : entity->getComponents()) {
                ...
                clientc->_clientUDP->addMessage(protocol::client::entityUpdate(clientc->_clientToken, entity->getId(),
                                                                               components).getBufferString());
                ...
                for (const std::string &key : components.getKeys()) {
                    ...
                    if (component) {
                        ...
                        try {
                            ...
                            logs::getLogger()[logs::ERRORS] << "Unable to find builders for '" << key << "'"
                                                            << std::endl;
                        }
                    }
                }
